...
const ApplicationHandOverStyle = styled.div`
  display: flex;
  ...
  min-width: 250px;
`;
const ApplicationHandOverHeaderStyle = styled.div`
  display: flex;
...
`;
const DialogApplicationHandOver = ({ title, onClose, UID, closeSlide }) => {
  ...
  return (
    <ApplicationHandOverStyle onClick={(e) => e.stopPropagation()}>
      <ApplicationHandOverHeaderStyle>
      ...
      <OfficeListStyle>
        {list.map((item, idx) => (
          ...
          >
            ...
            <TextSpanStyle size={10} color='#7e7777'>
              {item?.officeAddress}
            </TextSpanStyle>
          </OfficeItemStyle>
        ))}
      ...
      <Box>
        ...
        <ButtonUI onClick={clearSelect} variant='outline'>
          Отменить
        </ButtonUI>
      </Box>
    </ApplicationHandOverStyle>
  );
};

export default DialogApplicationHandOver;

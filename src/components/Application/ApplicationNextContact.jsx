...
const ApplicationNextContact = ({ onClose }) => {
  ...
  return (
    <ApplicationNextContactStyle onClick={(e) => e.stopPropagation()} onSubmit={handleSubmit(onSubmit)}>
      ...
      <InputUI
        register={register('nextDate', { required: 'Выберете дату', validate: { isAfter: (v) => moment().isSameOrBefore(v) || 'Дата не корректна' } })}
        type='date'
        width='300px'
      ...
      <LabelStyle>
        ...
        {
          errors?.comment &&
          <TextSpanStyle size={12} color='red'>{errors.comment.message}</TextSpanStyle>
        }
      </LabelStyle>

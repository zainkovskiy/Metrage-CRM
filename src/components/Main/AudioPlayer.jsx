...
const AudioPlayer = () => {
  ...
  const innerTime = (currentTime, duration) => {
    ...
    const durationMeconds = Math.round(duration - (durationHours * 3600) - (durationMinutes * 60));
    timeRef.current.innerHTML = `${getTime(minutes)}:${getTime(seconds)}/${getTime(durationMinutes)}:${getTime(durationMeconds)}`;
  }
  const setRageValue = () => {
  ...
  return (
    <AudioPlayerStyle>
      ...
      <RageStyle $back={back} ref={rageRef} id="cur-time" type="range" min="0" max='200' value={0} step="0" onChange={handleRage} />
